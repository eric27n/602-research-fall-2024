...
from tensorflow.keras.layers import Dense, Embedding, Input, Flatten, Concatenate, Multiply, Dropout
from tensorflow.keras.optimizers import Adagrad, Adam, SGD, RMSprop
from tensorflow.keras.regularizers import l2
from evaluate_new import evaluate_model
from Dataset import Dataset
from time import time
import GMF_new as GMF
...
def load_pretrain_model(model, gmf_model, mlp_model, num_layers):
    # MF embeddings
    gmf_user_embeddings = gmf_model.get_layer('mf_embedding_user').get_weights()
    gmf_item_embeddings = gmf_model.get_layer('mf_embedding_item').get_weights()
    model.get_layer('mf_embedding_user').set_weights(gmf_user_embeddings)
    model.get_layer('mf_embedding_item').set_weights(gmf_item_embeddings)
    
    # MLP embeddings
    mlp_user_embeddings = mlp_model.get_layer('mlp_embedding_user').get_weights()
    mlp_item_embeddings = mlp_model.get_layer('mlp_embedding_item').get_weights()
